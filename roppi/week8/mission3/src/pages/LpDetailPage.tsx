...
const LpDetailPage = () => {
  ...
  useEffect(() => {
  console.log("thumbnail:", lpData?.thumbnail);
  console.log("author avatar:", lpData?.author?.avatar);
}, [lpData]);


  // LP 좋아요
  const likes = lpData?.likes || [];
  ...
  const { mutate: postCommentMutate } = usePostComment(lpData, order);
