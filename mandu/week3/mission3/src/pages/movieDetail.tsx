        ...
        <div className="grid grid-rows-2 max-h-screen p-4">
            <section className="relative">
                <div className="absolute inset-0 text-white p-5 w-2lg">
                    ...
                    <p>{details?.overview.length > 200 ? `${details?.overview.slice(0, 200)}...`: details?.overview}</p>
                </div>
                <img className="w-full max-h-full object-cover" src={`${IMAGE_BASE_URL}${details?.poster_path}`} />
            </section>
            ...
            <section> 
                ...
                {casts && casts?.map((cast) => (
                    <li key={cast.id} className="flex justify-center justify-items-center rounded-xl bg-gray-200 m-2" >
                        <div className="flex flex-col items-center">
                            <img src={`${IMAGE_BASE_URL}${cast.profile_path}`} alt={cast.name} style={{ maxWidth: "50px", maxHeight: "50px" }} className="rounded-xl overflow-hidden " />
                            <div>
                                <p>{cast.character}</p>
