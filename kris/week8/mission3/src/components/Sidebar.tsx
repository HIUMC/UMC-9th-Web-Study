...
export const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
  ...
  return (
    <>
      ...
      />
      <aside
        className={`fixed transition-transform duration-200 flex flex-col justify-between bg-[#222222] text-white w-[200px] h-[calc(100dvh-60px)] z-40 top-[60px] left-0 ${
      ...
      >
        <div>
          <Link
            to="/search"
            onClick={onClose}
            className="flex flex-row my-3 px-5 py-3"
          >
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
          <div className="bg-[#555555] p-6 rounded-md w-[400px] h-[250px]">
            ...
            <div className="flex justify-center space-x-6">
              <button
                className="px-4 py-2 bg-gray-300 rounded-md cursor-pointer w-[100px]"
                onClick={handleDeleteUser}
              >
                예
              </button>
              <button
                className="px-4 py-2 bg-[#FF1E9D] text-white rounded-md cursor-pointer w-[100px]"
                onClick={() => setIsUserDeleteModalOpen(false)}
              >
                아니오
              </button>
