    ...
    <>
      ...
      <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[450px] bg-[#222222] rounded-2xl z-40">
        <div className="flex flex-row justify-between mt-6 mx-6">
          <h2 className="text-white text-xl">LP 추가하기</h2>
          <X
        ...
        <div className="flex flex-row justify-center my-10">
          <button onClick={handleLpImageClick}>
            <img
              className={`h-[250px] w-[250px] cursor-pointer rounded-full ${
                thumbnailUrl !== lpDefaultImage && `border-1 border-black`
              }`}
              src={thumbnailUrl}
              alt=""
            />
        ...
        <div className="flex flex-col space-y-4 p-4">
          <div>
            <input
              ...
              type="text"
              className="border-1 border-[#8a8a8a] rounded-md h-[35px] p-2 placeholder:text-[#8a8a8a] w-full text-white"
              placeholder="LP Name"
            />
          </div>
          <div>
          ...
          <div className="flex flex-row justify-between items-center space-x-2">
            ...
            <button
              className={`${
                tagInput.trim() ? enabledButtonStyle : disabledButtonStyle
              } h-[35px] p-3 rounded-md flex flex-col justify-center items-center`}
          ...
          <div className="w-full flex flex-wrap items-center gap-3">
            {tags.map((tag) => (
              <div
                key={tag.id}
                className="border-1 border-[#8a8a8a] flex flex-row rounded-md text-[#dfdfdf] p-1"
              >
          ...
          <div>
            <button
              className={`${
                nameInput.trim() && contentInput.trim() && tags.length
                  ? enabledButtonStyle
                  : disabledButtonStyle
              } w-full h-[40px] mt-2 rounded-md flex flex-col justify-center items-center text-white`}
