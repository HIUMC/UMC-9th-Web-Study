...
const MovieDetailPage = () => {
  ...
  const {data: movieDetail, isPending: isDetailPending, isError: isDetailError } = useCustomFetch<MovieDetail>({
    url: `https://api.themoviedb.org/3/movie/${movieId}?language=en-US`,
    deps: [movieId]
  });

  
  const {data: credit, isPending: isCreditPending, isError: isCreditError} = useCustomFetch<CreditsResponse>({
  ...
  // credit이 null일 수 있으므로 옵셔널 체이닝 사용
